<template>
    <van-form @submit="onSubmit" ref="formRef">
        <van-row justify="center" style="margin-top:30px;">
            <van-col style="font-size:16px">用户登录</van-col>
        </van-row>
        <van-cell-group inset>
            <van-field v-model="userInfo.name" name="用户名" label="用户名" placeholder="请输入用户名" :rules="[
            ...
            <van-field v-model="userInfo.checkpass" type="password" name="确认密码" label="确认密码" placeholder="请确认密码"
                :rules="[{validator: validatePass2}]" class="password" />
        </van-cell-group>
        ...
        <van-field name="checkbox" :rules="[{ validator: validatorMessage }]">
            <template #input>
...
const onSubmit = (values) => {
    formRef.value.validate().then(()=>{
        proxy.$axios.post("http://chengmei_dev.wanxikeji.cn/api/register",{name:userInfo.value.name,pw:userInfo.value.password,
            type:1}).then((res)=>{
